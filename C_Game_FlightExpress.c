...
void game_map_create(char** map)
{
    ...
    for (int i = 0; i < MAP_SIZE_ROW; i++)
    {
        ...
        {
            int random_seed = rand() % map_difficulty; // 이 값이 작을수록 난이도 어려움
            if (random_seed < 1)
            {
...
void game_map(char** map)
{
    for (int i = 0; i < MAP_SIZE_ROW - 27; i++) // 맨 처음 출력될 맵의 기준이 됨
    {
        ...
        // 충돌 판정, 충돌할 경우 life 1 감소, score 50 감소
        ...
        if (life <= 0)
        {
            ...
            printf("     ");
            ReleaseMutex(mutex); // 뮤텍스 언락
            SetEvent(g_hExitEvent); // 종료됨을 이벤트를 통해 알림
...
int main(void)
{
    ...
    while (1)
    {
        ...
        else if (select_menu == '3') // 비행기 색상, 난이도 조절
        {
            ...
            gotoxy(20, 12);
            printf("\t[Z] [C]::Change Speed(낮을수록 빠름)");
            gotoxy(20, 13);
            printf("\t[<] [>]::Change Map Difficulty(낮을수록 어려움)");
            gotoxy(20, 14);
            printf("\t    [Q]::Shutdown");
            textcolor(7);
            while (1)
